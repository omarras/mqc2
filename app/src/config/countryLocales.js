...
export function normalizeLocale(locale) {
    ...
    if (!lang || !region) return s;
    return `${lang.toLowerCase()}_${normalizeCountryCode(region)}`;
}

export function parseLocale(locale) {
    const l = normalizeLocale(locale);
    const [language, country] = l.split("_");
...
const localesToCountryCode = new Map();
...
for (const c of countryLocalesExpanded) {
    ...
    for (const loc of c.locales || []) {
        localesToCountryCode.set(normalizeLocale(loc), code);
    }
}

/* ------------------------------------------------------------------
   Existing functions (kept) + improved performance
------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------
   New helpers you’ll likely need
------------------------------------------------------------------ */

export function getCountryByLocale(locale) {
    const loc = normalizeLocale(locale);
    const code = localesToCountryCode.get(loc);
...
export function isSupportedLocale(locale) {
    return localesToCountryCode.has(normalizeLocale(locale));
}

export function getDefaultLocaleForCountry(code) {
    const c = getCountryByCode(code);
    return c?.locales?.[0] || null;
}

export function getAllSupportedLocales() {
    return Array.from(localesToCountryCode.keys()).sort();
}

export function getAllSupportedCountryCodes() {
    return Array.from(byCode.keys()).sort();
}

export function getAllSupportedLanguageCodes() {
    const set = new Set();
    for (const loc of getAllSupportedLocales()) {
        const { language } = parseLocale(loc);
        if (language) set.add(language); // already lower-case
    }
...
export const countryOptions = countryLocalesExpanded
    .map(c => ({ label: c.name, value: normalizeCountryCode(c.code) }))
...
export const localeOptions = (() => {
    ...
    for (const c of countryLocalesExpanded) {
        for (const loc of c.locales || []) {
            out.push({
                label: `${normalizeLocale(loc)} - ${c.name}`,
                value: normalizeLocale(loc)
            });
        }
    }
